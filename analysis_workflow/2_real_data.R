...
set.seed(23)
n1=27
d2=c(613.9,623.4,639.3,642.1,653.8,662.4,669.5,672.8,681.3,682.0,699.0,714.5,
     717.4,725.5,741.6,744.9,751.0,761.7,763.9,774.2,791.6,795.2,829.8,838.4,
     856.4,868.3,882.9)
d1=(d2-500)/100
d1
min_value <- min(d1)
...
L111=(4*n1/theta1^n1)-((2*n1*(1+mu1)^3)/(1+theta1*(1+mu1))^3)
L111
...
R12<- function(t, theta1, mu1) {
  ...
  numerator2 <- ((1 + theta1* (1 + mu1))^2) * (2 * theta1* exp(theta1* mu1) * (1 + mu1) + theta1^2 * exp(theta1* mu1) * mu1* (1 + mu1)) - 
    theta^2 * (1 + mu1) * exp(theta1* mu1) * 2 * ((1 + theta1* (1 + mu1)) * (1 + mu1))
  denominator2 <- (1 + theta1* (1 + mu1))^4
